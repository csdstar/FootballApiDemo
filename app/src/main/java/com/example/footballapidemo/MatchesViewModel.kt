...
class MatchesViewModel : ViewModel() {
    companion object {
        val competitions =
            listOf("全部", "英超PL", "欧冠CL", "法甲FL1", "欧联杯EC", "德甲BL1", "意甲SA")
    ...
    private val _pagesDataCache = mutableStateListOf<PageData>()

    init {
        // 初始化页面数据
        repeat(competitions.size) { _ ->
            val matchList = mutableStateListOf<Match>()
            _pagesData.add(PageData(matchList))
            _pagesDataCache.add(PageData(matchList))
        }
    }
    ...
    fun addMatch(index: Int, match: Match) {
        _pagesData[index].matchList.add(match)
    }

    fun addMatchCache(index: Int, match: Match) {
        _pagesDataCache[index].matchList.add(match)
    }
}